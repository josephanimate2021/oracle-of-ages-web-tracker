...
Well, it does, but the PopTracker pack has so many critical bugs that affect it's usability for new players of the Oracle of Ages Randomizer like Auto Tabbing between maps not working when connected to Archipelago (The client has that feature), 
listed events not being checked off properly (The client has that as well), Dungeon entrances not appearing each time you enter a dungeon with Dungeon Shuffle on (Again, AP client has that), and so on. 
